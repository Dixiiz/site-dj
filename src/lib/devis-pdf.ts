...
//  Bar/Club → Set DJ, Clé en main (Standard/Premium), Club, Afterwork, Association
...
export async function buildDevisPdf(
  quote: DevisQuoteData,
  opts: {
    contractNumber: string;
    validityDays: number;
    conditions?: string;
    /** Signature du client (devis signé) : apposée dans la case Bon pour accord */
    signature?: {
      name: string;
      dateIso: string;
      ip?: string | null;
      /** PNG (base64 data URL) dessiné par le client sur l'écran */
 ...
 "0,80 € / km",
  ...
  y -= 6;
  t("T.V.A non applicable, Article 293B du C.G.I",
    right("T.V.A non applicable, Article 293B du C.G.I", cMont, 8, r), y, 8, r, C.gris);
 ...
 "Possibilité de paiement total sans acompte par virement, chèque ou espèces.";
  const wrapCount = (txt: string, size: number, font: PDFFont, maxW: number) => {
    ...
    for (const w of words) {
      ...
      if (tw(test, size, font) > maxW) { n++; cur = w; } else cur = test;
    }
    return n;
  };
  const restH =
  ...
  if (sig) {
    const vert = rgb(0.13, 0.5, 0.25);
    const label = "SIGNÉ";
    const bw = b.widthOfTextAtSize(label, 11);
    const bx0 = W - M - bw - 34;
    page.drawRectangle({
      x: bx0, y: H - 86, width: bw + 42, height: 20,
      color: rgb(0.85, 0.95, 0.88), borderColor: rgb(0.2, 0.65, 0.35), borderWidth: 1,
    });
  ...
  if (sig) {
    ...
    if (drawn) {
      ...
      t(sig.name, M + 30, y - 43, 14, italic, rgb(0.1, 0.25, 0.55));
    }
    const d = new Date(sig.dateIso).toLocaleDateString("fr-FR");
    t(
      `Bon pour accord le ${d}`,
      right(`Bon pour accord le ${d}`, M + CW - 12, 8.5, r),
      y - 25, 8.5, r, C.texte
    );
    t("Signature électronique nominative (espace client)", M + 12, y - 51, 7, r, C.gris);
  } else {
    t("Date : ____ / ____ / ________",
      right("Date : ____ / ____ / ________", M + CW - 12, 8.5, r), y - 25, 8.5, r, C.gris);
  }
  page.drawLine({ start: { x: M + 12, y: y - 47 }, end: { x: M + 250, y: y - 47 }, thickness: 0.6, color: C.grisLigne });

  y -= 68;
  t(`Fait en deux exemplaires, à Huisseau-sur-Cosson, le ${new Date().toLocaleDateString("fr-FR")}.`,
    M, y, 8, r, C.gris);

...
exports.buildContratPdf = buildContratPdf;
...
async function buildContratPdf(quote, opts) {
    ...
    const t = (txt, x, yy, size, font, color = C.texte) => page.drawText(txt, { x, y: yy, size, font, color });
    ...
    const rows = [
        ...
        ["Type de prestation", quote.event_type || "-"],
        ["Date", dateLong],
    ...
    for (const art of ALL_ARTICLES) {
        section(art.titre);
        for (const p of art.paragraphes)
            para(p);
        y -= 4;
    }
    ...
    if (y < 160)
        newPage();
    y = 150; // on ancre les signatures en bas de page
    ...
    t("ORGANISATEUR", M + 12, yTop + 50, 8, b, C.gris);
    t("Signature et mention : Lu et approuvé", M + 12, yTop + 38, 8.5, r, C.gris);
    if (sig) {
        let drawn = null;
        ...
        if (drawn) {
            const w = 110, h2 = Math.min((drawn.height / drawn.width) * w, 28);
            page.drawImage(drawn, { x: M + 14, y: yTop + 6, width: w, height: h2 });
        }
        else {
            const italic = await doc.embedFont(pdf_lib_1.StandardFonts.TimesRomanItalic);
            t(sig.name, M + 24, yTop + 14, 13, italic, (0, pdf_lib_1.rgb)(0.1, 0.25, 0.55));
        }
        const d = new Date(sig.dateIso).toLocaleDateString("fr-FR");
        t(`Lu et approuvé le ${d}`, right(`Lu et approuvé le ${d}`, M + half - 12, 8, r), yTop + 38, 8, r, C.texte);
    }
    else {
        page.drawLine({ start: { x: M + 12, y: yTop + 12 }, end: { x: M + half - 30, y: yTop + 12 }, thickness: 0.6, color: C.grisLigne });
    }
    // droite : prestataire
    const rx2 = M + half + 20;
    page.drawRectangle({ x: rx2, y: yTop, width: half, height: boxH, borderColor: C.grisLigne, borderWidth: 0.8 });
    t("PRESTATAIRE", rx2 + 12, yTop + 50, 8, b, C.gris);
    t("SOULAINE Maxime (Propul'Sound DJ)", rx2 + 12, yTop + 38, 8.5, b, C.texte);
    if (sigImg) {
        const sigW = 90, sigH = Math.min((sigImg.height / sigImg.width) * sigW, 20);
        page.drawImage(sigImg, { x: rx2 + half - sigW - 14, y: yTop + 5, width: sigW, height: sigH });
    }
    else {
        page.drawLine({ start: { x: rx2 + 12, y: yTop + 12 }, end: { x: rx2 + half - 30, y: yTop + 12 }, thickness: 0.6, color: C.grisLigne });
    }
    y = yTop - 14;
    if (y < 30)
        newPage();
